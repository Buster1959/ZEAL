# ZEAL-Heat Troubleshooting

> **Archived repository snapshot.** The maintained source of truth is now the
> [ZEAL-Heat wiki Troubleshooting guide](https://github.com/Buster1959/ZEAL-Heat/wiki/Troubleshooting).
...
ZEAL-Heat-owned room thermostats are intentionally excluded from physical equipment
pickers. Reload ZEAL-Heat after changing registry assignments.

## A saved change is rejected as stale

Another browser or process saved newer data. Reload the ZEAL-Heat page, review the
...
Check the control sources shown in ZEAL-Heat. Active Away mode takes priority over
...
canonical ZEAL-Heat thermostat.

## One physical thermostat did not follow another in the same room

Update to ZEAL-Heat 0.13.4 or later. A manual setpoint change on any configured
physical TRV is the new room target and is propagated to the canonical ZEAL-Heat
thermostat and every other physical TRV in that room. ZEAL-Heat's loop guard ignores
only the immediate echo of its own service call, not a later manual selection of
the same temperature. If a physical TRV is unavailable during propagation, ZEAL-Heat
...
and delayed or out-of-order radio acknowledgements are recognised as ZEAL-Heat's own
writes rather than fed back into the room. ZEAL-Heat waits until a physical dial has
...
Confirm **Setup → ZEAL-Heat Learning → Enable Schedule Adaptation** is enabled. ZEAL-Heat
...
A ZEAL-Heat-generated write, a restoration to the scheduled target or a change made
...
canonical ZEAL-Heat thermostat and was not merely ZEAL-Heat acknowledging its own write.
...
ZEAL-Heat rechecks the affected room and schedule period immediately before writing.
If that period changed after the proposal or acceptance, ZEAL-Heat stops instead of
...
## A standard user cannot see ZEAL-Heat in the sidebar

Standard-user panel access requires ZEAL-Heat `0.13.6` or later. Confirm that version
...
panel list. Also confirm **Setup → Home Assistant sidebar → Show ZEAL-Heat in the Home
...
Check the zone's Manual override entity first. When it is on, ZEAL-Heat deliberately
...
sensors currently reports `on`. ZEAL-Heat intentionally excludes that room from
...
## ZEAL-Heat reports an entity health warning
...
entity's Home Assistant `last_reported` time passed ZEAL-Heat's four-hour stale
...
If it happens again, do not recreate the zone: reload the ZEAL-Heat page and attach
redacted diagnostics with the exact save sequence to the issue report.

## Enable diagnostic logging

Use **Settings → Devices & Services → ZEAL-Heat HVAC System → Enable debug logging**
for a temporary trace. Reproduce the problem, then disable logging and download
diagnostics. The configuration and audit downloads under **ZEAL-Heat → Setup** are
...
Open an issue at <https://github.com/Buster1959/ZEAL-Heat/issues> with the ZEAL-Heat
